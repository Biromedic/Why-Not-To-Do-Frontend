...
export default function Task({ id, description, complete, onToggle, onTrash, onRename }) {
  const [editMode, setEditMode] = useState(false);

  console.log("Task props:", { id, description, complete });

  return (
    <div className={'task ' + (complete ? 'done' : '')}>
      <Checkbox checked={complete} onClick={() => onToggle(!complete)} />
      {!editMode && (
